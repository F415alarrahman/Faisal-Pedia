...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
  ...
  if ($qtyBaru <= 0) {
    $response["value"]   = 0;
    $response["message"] = "Qty tidak boleh kurang dari 1!";
    echo json_encode($response);
    exit;
  }
  ...
  try {
    ...
    $sqlUpdateItem = "
      UPDATE order_items 
       ...
       WHERE id_item = $id_item_int
    ";
    if (!mysqli_query($con, $sqlUpdateItem)) {
      throw new Exception("Gagal update item!");
    }
    ...
    $response["value"]        = 1;
    $response["message"]      = "Item berhasil diupdate.";
